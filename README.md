...
新生成的配置默认使用 `🛟 自动故障切换`：Reality 正常时行为不变，连接失败时按 Reality → CDN → Hysteria2 → AnyTLS 顺序切换。
`US-Reality-WARP` 会进入自动测速和手动选择，但不进入自动故障切换；测速策略会根据实测延迟决定是否使用它，默认故障链路不会被 WARP 拖慢。
